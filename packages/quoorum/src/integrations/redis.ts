...
export async function initRedis(): Promise<RedisClientType | null> {
  ...
  try {
    ...
    redisClient.on('error', (err: Error) => {
      quoorumLogger.error('Redis client error', err, {})
      isConnected = false
    })

    redisClient.on('connect', () => {
      quoorumLogger.info('Redis connected', {})
      isConnected = true
    })
    ...
    quoorumLogger.warn('Redis not available, using in-memory cache', { error: String(error) })
    return null
  }
}
