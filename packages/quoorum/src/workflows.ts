...
export async function executeWorkflow(workflow: Workflow): Promise<boolean> {
  quoorumLogger.info('Executing workflow', { name: workflow.name, workflowId: workflow.id })
  ...
  try {
    ...
    quoorumLogger.error('Workflow execution failed', error instanceof Error ? error : new Error(String(error)), { workflowId: workflow.id, workflowName: workflow.name })
    return false
  }
}

function executeAction(action: WorkflowAction): Promise<void> {
  quoorumLogger.info('Executing workflow action', { type: action.type, params: action.params })
