...
class DebugLogger {
  ...
  private log(level: LogLevel, message: string, data?: unknown) {
    ...
    // Use quoorumLogger for structured logging
    const context = data ? { debugData: data } : undefined

    switch (level) {
      case LogLevel.DEBUG:
        quoorumLogger.debug(message, context)
        break
      case LogLevel.INFO:
        quoorumLogger.info(message, context)
        break
      case LogLevel.WARN:
        quoorumLogger.warn(message, context)
        break
      case LogLevel.ERROR:
        quoorumLogger.error(
          message,
...
class PerformanceMonitor {
  ...
  start(label: string) {
    ...
    quoorumLogger.debug(`${label}: ${duration}ms`, { label, duration })
    return duration
  }
...
class ErrorTracker {
  ...
  track(error: Error, context?: Record<string, unknown>) {
    ...
    quoorumLogger.error(error.message, error, context)
  }
...
export function prettyPrintDebate(debate: DebateResult) {
  ...
  // Use quoorumLogger for structured output
  quoorumLogger.info('Debate debug info', { debugInfo })
}
