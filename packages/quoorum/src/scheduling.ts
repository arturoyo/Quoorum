...
async function executeScheduledDebate(id: string) {
  ...
  try {
    ...
    quoorumLogger.info('Scheduled debate completed', { id, debateId: scheduled.result?.sessionId })
  } catch (error) {
...
async function executeRecurringDebate(id: string) {
  ...
  try {
    ...
    quoorumLogger.info('Recurring debate completed', { id, nextRun: recurring.nextRun.toISOString() })
  } catch (error) {
    quoorumLogger.error(
