...
Update `packages/quoorum/src/rate-limiting.ts`:
...
Update `packages/quoorum/src/caching.ts`:
...
1. Add indexes:
   ```sql
   CREATE INDEX idx_debates_user ON quoorum_debates(user_id);
   CREATE INDEX idx_debates_status ON quoorum_debates(status);
   CREATE INDEX idx_debates_created ON quoorum_debates(created_at);
   ```
