    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const consultations = await db
          .select({
            id: forumConsultations.id,
            originalMessage: forumConsultations.originalMessage,
            triggers: forumConsultations.triggers,
            urgency: forumConsultations.urgency,
            strategy: forumConsultations.strategy,
            responseApproach: forumConsultations.responseApproach,
            recommendHumanEscalation: forumConsultations.recommendHumanEscalation,
            adviceConfidence: forumConsultations.adviceConfidence,
            createdAt: forumConsultations.createdAt,
            clientId: forumConsultations.clientId,
            clientName: clients.name,
          })
          .from(forumConsultations)
          .leftJoin(clients, eq(forumConsultations.clientId, clients.id))
          .where(eq(forumConsultations.userId, ctx.user.id))
          .orderBy(desc(forumConsultations.createdAt))
          .limit(limit)
  ...
  getStats: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const [totalResult] = await db
        ...
        .where(
          and(
            eq(forumConsultations.userId, ctx.user.id),
            gte(forumConsultations.createdAt, thirtyDaysAgo)
          )
        )
      ...
      const urgencyStats = await db
        ...
        .groupBy(forumConsultations.urgency)
      ...
      const [escalationsResult] = await db
        ...
        .where(
          and(
            eq(forumConsultations.userId, ctx.user.id),
            eq(forumConsultations.recommendHumanEscalation, true),
            gte(forumConsultations.createdAt, thirtyDaysAgo)
          )
        )
      ...
      const [avgConfidence] = await db
        .select({
          avg: sql<string>`AVG(${forumConsultations.adviceConfidence})`,
        })
        .from(forumConsultations)
        .where(
          and(
  ...
  store: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const [consultation] = await db
        ...
        .returning({ id: forumConsultations.id })
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const [consultation] = await db
        .select()
        .from(forumConsultations)
        .where(and(eq(forumConsultations.id, input.id), eq(forumConsultations.userId, ctx.user.id)))
