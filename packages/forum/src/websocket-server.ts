...
export class ForumWebSocketServer {
  ...
  constructor(port: number = 3001) {
    ...
    this.wss.on('connection', (ws: WebSocket, request: IncomingMessage) => {
      forumLogger.info('WebSocket client connected', { remoteAddress: request.socket.remoteAddress })
      ...
      ws.on('message', (data: Buffer) => {
        try {
          ...
          forumLogger.error('WebSocket message parse error', error instanceof Error ? error : new Error(String(error)), {})
        }
      })

      ws.on('close', () => {
        forumLogger.info('WebSocket client disconnected', {})
        this.handleDisconnect(ws)
      })

      ws.on('error', (error: Error) => {
        forumLogger.error('WebSocket client error', error, {})
      })
    ...
    forumLogger.info('WebSocket server listening', { port })
  }

  private handleMessage(ws: WebSocket, message: unknown) {
    if (typeof message !== 'object' || message === null) {
      forumLogger.error('Invalid WebSocket message format', new Error('Invalid message format'), { message })
      return
    }
    ...
    if (!type) {
      forumLogger.error('WebSocket message missing type', new Error('Message missing type'), { message })
      return
    }
    ...
    switch (type) {
      case 'subscribe':
        if (!debateId) {
          forumLogger.error('Subscribe message missing debateId', new Error('Subscribe message missing debateId'), {})
          return
        }
      ...
      case 'unsubscribe':
        if (!debateId) {
          forumLogger.error('Unsubscribe message missing debateId', new Error('Unsubscribe message missing debateId'), {})
          return
        }
      ...
      default:
        forumLogger.warn('Unknown WebSocket message type', { type })
    }
  }
  ...
  private subscribe(ws: WebSocket, debateId: string) {
    ...
    forumLogger.info('Client subscribed to debate', { debateId })
  ...
  private unsubscribe(ws: WebSocket, debateId: string) {
    ...
    forumLogger.info('Client unsubscribed from debate', { debateId })
  ...
  public broadcastDebateUpdate(update: DebateUpdate) {
    ...
    forumLogger.info('WebSocket broadcast sent', { type, subscriberCount: subscribers.size, debateId })
  }
  ...
  public close() {
    this.wss.close()
    forumLogger.info('WebSocket server closed', {})
  }
}
...
export function getWebSocketServer(): ForumWebSocketServer {
  if (!wsServer) {
    const port = parseInt(process.env['FORUM_WS_PORT'] || '3001', 10)
    wsServer = new ForumWebSocketServer(port)
  }
  return wsServer
}
