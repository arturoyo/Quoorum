...
export class DebateReplayer {
  ...
  async replay(options: ReplayOptions = {}): Promise<void> {
    ...
    forumLogger.debug('Starting debate replay', {
    ...
    for (let r = 0; r < this.result.rounds.length; r++) {
      ...
      forumLogger.debug(`Round ${r + 1} started`, {
        sessionId: this.result.sessionId,
        round: r + 1,
      ...
      for (let m = 0; m < round.messages.length; m++) {
        ...
        if (options.pauseOnIntervention && message.agentKey === 'meta_moderator') {
          forumLogger.debug('Paused on meta-moderator intervention', {
            sessionId: this.result.sessionId,
            round: r + 1,
          })
          this.isPaused = true
    ...
    forumLogger.info('Debate replay complete', {
      sessionId: this.result.sessionId,
  ...
  jumpToRound(round: number): void {
    if (round >= 0 && round < this.result.rounds.length) {
      forumLogger.debug('Jumped to round', {
        sessionId: this.result.sessionId,
        round: round + 1,
      })
    }
  ...
  togglePause(): void {
    ...
    forumLogger.debug(this.isPaused ? 'Replay paused' : 'Replay resumed', {
      sessionId: this.result.sessionId,
  ...
  private async displayMessage(message: DebateMessage, options: ReplayOptions): Promise<void> {
    const expertName = message.agentKey
      ...
      .join(' ')
    forumLogger.debug('Displaying message in replay', {
      sessionId: this.result.sessionId,
...
export function exampleStreamUsage(): void {
  ...
  streamer.subscribe((event) => {
    switch (event.type) {
      case 'round_start': {
        ...
        forumLogger.debug('Streamer: Round started', {
          round: data.round,
          total: data.total,
        })
        break
      ...
      case 'message': {
        ...
        forumLogger.debug('Streamer: Message', {
      ...
      case 'intervention': {
        ...
        forumLogger.info('Streamer: Meta-moderator intervention', {
          messagePreview: data.message.content.substring(0, 100),
        })
        break
      }
      case 'quality_check': {
        ...
        forumLogger.debug('Streamer: Quality check', {
      ...
      case 'round_end': {
        ...
        forumLogger.debug('Streamer: Round ended', {
          round: data.round,
        })
        break
      }
      case 'debate_end': {
        ...
        forumLogger.info('Streamer: Debate ended', {
  ...
  forumLogger.debug('Streamer example setup complete')
}
...
export function printTimeline(result: DebateResult): void {
  ...
  forumLogger.debug('Debate timeline', {
