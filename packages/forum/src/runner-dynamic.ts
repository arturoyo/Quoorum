...
export async function runDebate(options: RunDebateOptions): Promise<DebateResult> {
  ...
  try {
    ...
    forumLogger.info('Starting debate', { mode: debateMode.mode, sessionId })
    ...
    forumLogger.info('Debate completed', {
      sessionId,
    ...
    forumLogger.error('Debate failed', new Error(errorMessage), { sessionId })
...
async function determineDebateMode(
  ...
  if (forceMode === 'static') {
    return {
      ...
      agents: Object.values(FORUM_AGENTS),
...
async function generateAgentResponse(input: GenerateAgentResponseInput): Promise<DebateMessage> {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error)
    forumLogger.error(`Agent ${agent.name} failed`, new Error(errorMessage), { sessionId, agentName: agent.name })
