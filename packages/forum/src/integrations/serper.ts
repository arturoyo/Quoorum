...
export async function searchWeb(
  ...
  if (!SERPER_API_KEY) {
    forumLogger.warn('SERPER_API_KEY not set, web search disabled', {})
    return []
  }
...
export async function searchNews(
  ...
  if (!SERPER_API_KEY) {
    forumLogger.warn('SERPER_API_KEY not set, news search disabled', {})
    return []
  }
...
export async function searchWebCached(
  ...
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    forumLogger.debug('Using cached search results', { query, cacheKey })
