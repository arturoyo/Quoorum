...
export const forumReportStatusEnum = pgEnum('quoorum_report_status', [
  'pending',
  'generating',
  'completed',
  'failed',
...
export const forumReportFormatEnum = pgEnum('quoorum_report_format', [
  'pdf',
  'html',
  'markdown',
])
...
export const quoorumReports = pgTable('quoorum_reports', {
  ...
  // Report info
  type: quoorumReportTypeEnum('type').notNull(),
  ...
  userIdx: index('idx_quoorum_reports_user').on(table.userId),
  typeIdx: index('idx_quoorum_reports_type').on(table.type),
  statusIdx: index('idx_quoorum_reports_status').on(table.status),
  createdAtIdx: index('idx_quoorum_reports_created').on(table.createdAt),
}))
...
export const forumScheduledReports = pgTable('quoorum_scheduled_reports', {
  ...
  userIdx: index('idx_quoorum_scheduled_reports_user').on(table.userId),
  activeIdx: index('idx_quoorum_scheduled_reports_active').on(table.isActive),
  nextRunIdx: index('idx_quoorum_scheduled_reports_next').on(table.nextRunAt),
}))
...
export const quoorumReportsRelations = relations(quoorumReports, ({ one }) => ({
  user: one(profiles, {
    fields: [quoorumReports.userId],
