...
export const forumSessions = pgTable(
  ...
  (table) => ({
    userIdx: index('idx_forum_sessions_user').on(table.userId),
    statusIdx: index('idx_forum_sessions_status').on(table.status),
    createdAtIdx: index('idx_forum_sessions_created').on(table.createdAt),
  })
)
...
export const forumMessages = pgTable(
  ...
  (table) => ({
    sessionIdx: index('idx_forum_messages_session').on(table.sessionId),
    roundIdx: index('idx_forum_messages_round').on(table.sessionId, table.round),
  })
)
...
export const forumContextSources = pgTable(
  ...
  (table) => ({
    sessionIdx: index('idx_forum_context_session').on(table.sessionId),
  })
)
...
export const forumTranslations = pgTable(
  ...
  (table) => ({
    messageIdx: index('idx_forum_translations_message').on(table.messageId),
  })
)
 ...
 */
export const FORUM_AGENTS: Record<string, ForumAgentConfig> = {
  optimizer: {
    key: 'optimizer',
    name: 'Optimista',
    role: 'optimizer',
