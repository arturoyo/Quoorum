 ...
 */
export const forumApiKeys = pgTable('forum_api_keys', {
 ...
 */
export const forumWebhooks = pgTable('forum_webhooks', {
  id: uuid('id').defaultRandom().primaryKey(),
  userId: uuid('user_id')
    .notNull()
    .references(() => profiles.id, { onDelete: 'cascade' }),
 ...
 */
export const forumWebhookLogs = pgTable('forum_webhook_logs', {
  id: uuid('id').defaultRandom().primaryKey(),
...
export type ForumApiKey = typeof forumApiKeys.$inferSelect
export type NewForumApiKey = typeof forumApiKeys.$inferInsert
