...
# 5. Verificar y liberar puerto 3005 si esta en uso
Write-Host "[5/5] Verificando puerto 3005..." -ForegroundColor Yellow
try {
    ...
    if ($port3005) {
        ...
        if ($process) {
            Write-Host "   [WARN] Puerto 3005 en uso por proceso: $($process.ProcessName) (PID: $processId)" -ForegroundColor Yellow
            Write-Host "   [INFO] Liberando puerto 3005..." -ForegroundColor Gray
            try {
                Stop-Process -Id $processId -Force -ErrorAction Stop
                Start-Sleep -Milliseconds 500
                Write-Host "   [OK] Puerto 3005 liberado" -ForegroundColor Green
            } catch {
        ...
        Write-Host "   [OK] Puerto 3005 disponible" -ForegroundColor Green
    }
} catch {
