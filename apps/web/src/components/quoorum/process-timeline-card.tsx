...
export function ProcessTimelineCard({ process }: ProcessTimelineCardProps) {
  ...
  return (
    <Card className="bg-[#111b21] border-[#2a3942] hover:border-purple-500/30 transition-colors">
      <CardContent className="p-4">
              ...
              <div key={phase.phaseNumber || index} className="flex items-start gap-3">
                ...
                <div
                  className={cn(
                    ...
                    isPending && 'bg-[#2a3942] text-[#8696a0] border border-[#2a3942]',
                    phaseStatus === 'skipped' && 'bg-[#2a3942] text-[#8696a0] opacity-50'
                  )}
                >
                  {isCompleted ? (
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={cn(
                      ...
                      isPending && 'text-[#8696a0]',
                      phaseStatus === 'skipped' && 'text-[#8696a0] line-through'
                    )}
                  >
                    {phase.name}
                  </p>
                  {phase.description && (
                    <p className="text-xs text-[#8696a0] mt-1 line-clamp-2">
                      {phase.description}
                    </p>
                  )}
                  {phase.completedAt && (
        ...
        {/* Footer */}
        <div className="mt-4 pt-3 border-t border-[#2a3942]">
