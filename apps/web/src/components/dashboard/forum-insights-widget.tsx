...
interface ForumInsightsWidgetProps extends BaseWidgetProps {
  compact?: boolean
}
...
export function ForumInsightsWidget({ compact = false, className }: ForumInsightsWidgetProps) {
  const {
    data: recent,
    isLoading: recentLoading,
    error: recentError,
  } = api.forumInsights.getRecent.useQuery({ limit: compact ? 3 : 5 })

  const { data: stats } = api.forumInsights.getStats.useQuery()
