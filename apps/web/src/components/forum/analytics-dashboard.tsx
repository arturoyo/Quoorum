...
export function AnalyticsDashboard() {
  ...
  const { isLoading: isLoadingAnalytics } = api.forum.analytics.useQuery()
  const { isLoading: isLoadingExperts } = api.forum.expertLeaderboard.useQuery()

  // Fetch Phase 4 data (topExperts not used yet - reserved for future feature)
  void api.forumFeedback.getTopExperts.useQuery({ limit: 5 })
  const { data: influenceStats } = api.forumDeals.getInfluenceStats.useQuery()
