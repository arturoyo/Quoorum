...
function LinkDebateDialog({ dealId, onSuccess }: { dealId: string; onSuccess: () => void }) {
  ...
  const { data: debates, isLoading } = api.forum.list.useQuery(
    { limit: 20, orderBy: 'recent' }
  )

  const linkDebate = api.forumDeals.linkDebate.useMutation({
    onSuccess: () => {
...
function RecommendationsSection({ dealId }: { dealId: string }) {
  const { data: recommendations, isLoading } = api.forumDeals.getRecommendations.useQuery({
    dealId,
  })

  const generateRecommendations = api.forumDeals.generateRecommendations.useMutation({
    onSuccess: () => {
  ...
  const dismissRecommendation = api.forumDeals.dismissRecommendation.useMutation({
    onSuccess: () => {
...
export function DealDebateWidget({
  ...
  } = api.forumDeals.getDebatesByDeal.useQuery({ dealId: entityId })
  ...
  } = api.forumDeals.getDealsByDebate.useQuery({ debateId: entityId })

  const unlinkDebate = api.forumDeals.unlinkDebate.useMutation({
    onSuccess: () => {
  ...
  const updateInfluence = api.forumDeals.updateInfluence.useMutation({
    onSuccess: () => {
...
export function SuggestedDealsForForum() {
  const { data: suggestedDeals, isLoading } = api.forumDeals.getSuggestedDeals.useQuery({
    limit: 5,
  })
