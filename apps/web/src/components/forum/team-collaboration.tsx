...
export function TeamCollaboration({ debateId }: TeamCollaborationProps) {
  ...
  // Fetch comments
  const { data: comments, isLoading } = api.forum.comments.list.useQuery({ debateId })

  // Fetch team members
  const { data: teamMembers } = api.forum.team.list.useQuery()

  // Create comment mutation
  const createComment = api.forum.comments.create.useMutation({
    onSuccess: () => {
      setComment('')
      // Refetch comments
    },
