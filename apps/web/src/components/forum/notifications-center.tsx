...
function PreferencesPanel() {
  const { data: preferences, isLoading } = api.forumNotifications.getPreferences.useQuery()
  const updatePreferences = api.forumNotifications.updatePreferences.useMutation({
    onSuccess: () => {
...
export function NotificationsCenter({
  ...
  const {
    ...
    isLoading,
    refetch,
  } = api.forumNotifications.list.useQuery({
    limit: 50,
    unreadOnly: activeTab === 'unread',
  })

  const { data: unreadCount } = api.forumNotifications.getUnreadCount.useQuery()

  const markAsRead = api.forumNotifications.markAsRead.useMutation({
    onSuccess: () => {
      void refetch()
    },
  })

  const markAllAsRead = api.forumNotifications.markAllAsRead.useMutation({
    onSuccess: () => {
  ...
  const archive = api.forumNotifications.archive.useMutation({
    onSuccess: () => {
      void refetch()
    },
  })

  const archiveAllRead = api.forumNotifications.archiveAllRead.useMutation({
    onSuccess: () => {
