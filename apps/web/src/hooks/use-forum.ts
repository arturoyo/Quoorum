...
export function useDebateList(refreshInterval?: number) {
  const { data: debates, isLoading, error, refetch } = trpc.forum.list.useQuery()
...
export function useDebate(debateId: string | null) {
  const {
    ...
    error,
    refetch,
  } = trpc.forum.get.useQuery({ id: debateId! }, { enabled: !!debateId })
...
export function useStartDebate() {
  const utils = trpc.useUtils()
  const startMutation = trpc.forum.start.useMutation({
    onSuccess: () => {
      utils.forum.list.invalidate()
    },
  })
...
export function useForumAnalytics() {
  const { data: analytics, isLoading, error } = trpc.forum.analytics.useQuery()
...
export function useExpertPerformance(expertId?: string) {
  const {
    ...
    isLoading,
    error,
  } = trpc.forum.expertPerformance.useQuery({ expertId: expertId! }, { enabled: !!expertId })
...
export function useAddReaction() {
  const utils = trpc.useUtils()
  const addReactionMutation = trpc.forum.addReaction.useMutation({
    onSuccess: (_, variables) => {
      utils.forum.get.invalidate({ id: variables.debateId })
    },
  })
...
export function useCustomExperts() {
  const { data: experts, isLoading, error, refetch } = trpc.forum.listExperts.useQuery()
  const utils = trpc.useUtils()

  const createMutation = trpc.forum.createCustomExpert.useMutation({
    onSuccess: () => {
      utils.forum.listExperts.invalidate()
    },
  })

  const updateMutation = trpc.forum.updateCustomExpert.useMutation({
    onSuccess: () => {
      utils.forum.listExperts.invalidate()
    },
  })

  const deleteMutation = trpc.forum.deleteCustomExpert.useMutation({
    onSuccess: () => {
      utils.forum.listExperts.invalidate()
    },
  })
...
export function useRateLimit() {
  const { data: rateLimit, isLoading, error, refetch } = trpc.forum.checkRateLimit.useQuery()
...
export function useSimilarDebates(question: string, enabled: boolean = true) {
  const {
    ...
    isLoading,
    error,
  } = trpc.forum.getSimilar.useQuery({ question }, { enabled: enabled && question.length > 10 })
...
export function useDebateSearch(initialQuery: string = '') {
  ...
  const { data: debates } = trpc.forum.list.useQuery()
